...
export function BlockEditor({ block, onSave, onClose }: Props) {
  ...
  const set = (k: keyof BioBlock, v: unknown) => setData((d) => ({ ...d, [k]: v }));
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-4 sm:p-6">
      ...
      >
        {/* Header */}
        <div className="flex items-center justify-between px-5 py-4 border-b sticky top-0 z-10" style={{ borderColor: "hsl(var(--border))", background: "hsl(var(--background))" }}>
          <h2 className="text-base font-bold" style={{ color: "hsl(var(--foreground))" }}>
            {block?.id ? "Editar bloque" : "Nuevo bloque"}
          </h2>
          <button type="button" onClick={onClose} className="p-1.5 rounded-lg hover:opacity-70 transition-opacity">
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-5 p-5">
          ...
          {data.type === "button" && (
            <>
              <div>
                <Label>Título *</Label>
                <input className={inputCls} style={inputStyle} required value={data.title ?? ""} onChange={(e) => set("title", e.target.value)} placeholder="WhatsApp" />
              </div>
              <div>
                <Label>URL *</Label>
                <input className={inputCls} style={inputStyle} required type="url" value={data.url ?? ""} onChange={(e) => set("url", e.target.value)} placeholder="https://wa.me/..." />
              </div>
              <div>
                <Label>Ícono</Label>
                    ...
                    return (
                      <button
                        ...
                        className={`w-8 h-8 rounded-lg flex items-center justify-center transition-all ${
                          data.icon_name === name ? "ring-2 ring-[hsl(var(--primary))]" : "opacity-50 hover:opacity-80"
                        }`}
                        style={{ background: "hsl(var(--muted))" }}
                      >
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <div className="flex items-center gap-2">
                    <input type="color" value={data.bg_color ?? "#4361ee"} onChange={(e) => set("bg_color", e.target.value)} className="w-8 h-8 rounded cursor-pointer border-0 p-0" />
                    <input className={`${inputCls} flex-1`} style={inputStyle} value={data.bg_color ?? "#4361ee"} onChange={(e) => set("bg_color", e.target.value)} placeholder="#4361ee" />
                  </div>
                </div>
                <div>
                  <Label>Color de texto</Label>
                  <div className="flex items-center gap-2">
                    <input type="color" value={data.text_color ?? "#ffffff"} onChange={(e) => set("text_color", e.target.value)} className="w-8 h-8 rounded cursor-pointer border-0 p-0" />
                    <input className={`${inputCls} flex-1`} style={inputStyle} value={data.text_color ?? "#ffffff"} onChange={(e) => set("text_color", e.target.value)} placeholder="#ffffff" />
                  </div>
                </div>
              </div>
            </>
          ...
          {data.type === "card" && (
            <>
              <div>
                <Label>Título *</Label>
                <input className={inputCls} style={inputStyle} required value={data.title ?? ""} onChange={(e) => set("title", e.target.value)} placeholder="MisTrajes" />
              </div>
              <div>
                <Label>Subtítulo</Label>
                <input className={inputCls} style={inputStyle} value={data.subtitle ?? ""} onChange={(e) => set("subtitle", e.target.value)} placeholder="Software de alquiler" />
              </div>
              <div>
                <Label>Descripción</Label>
                <textarea className={`${inputCls} resize-none`} style={inputStyle} rows={2} value={data.content ?? ""} onChange={(e) => set("content", e.target.value)} placeholder="Descripción breve del proyecto…" />
              </div>
              <div>
                <Label>URL (opcional)</Label>
                <input className={inputCls} style={inputStyle} type="url" value={data.url ?? ""} onChange={(e) => set("url", e.target.value)} placeholder="https://..." />
              </div>
              <div>
                <Label>Imagen</Label>
          ...
          {data.type === "text" && (
            <>
              <div>
                ...
                <input className={inputCls} style={inputStyle} value={data.title ?? ""} onChange={(e) => set("title", e.target.value)} placeholder="Nuestros servicios" />
              </div>
              <div>
          ...
          {data.type === "video" && (
            <>
              <div>
                ...
                <input className={inputCls} style={inputStyle} value={data.title ?? ""} onChange={(e) => set("title", e.target.value)} placeholder="Nuestro demo" />
              </div>
              <div>
          ...
          {data.type === "divider" && (
            <div>
              ...
              <input className={inputCls} style={inputStyle} value={data.title ?? ""} onChange={(e) => set("title", e.target.value)} placeholder="Mis proyectos" />
            </div>
          )}
          ...
          <div className="flex items-center justify-between py-2 border-t" style={{ borderColor: "hsl(var(--border))" }}>
            <div>
              <p className="text-sm font-medium" style={{ color: "hsl(var(--foreground))" }}>Activo</p>
              <p className="text-xs" style={{ color: "hsl(var(--muted-foreground))" }}>Visible en la página pública</p>
            </div>
            <button
              type="button"
              onClick={() => set("is_active", !data.is_active)}
              className={`w-11 h-6 rounded-full transition-all flex items-center ${data.is_active ? "justify-end bg-[hsl(var(--primary))]" : "justify-start bg-[hsl(var(--muted))]"} px-0.5`}
            >
          ...
          {error && (
            <p className="text-xs px-3 py-2 rounded-lg" style={{ background: "hsl(0 84% 60% / 0.1)", color: "hsl(0 84% 60%)" }}>
              {error}
            </p>
          )}
